...
optimizer = optim.SGD(net.parameters(), lr=0.002, momentum=0.9)   #optim模块中的SGD梯度优化方式---随机梯度下降
...
transform_form_0 = transforms.Compose(
...
trainset_0 = torchvision.datasets.CIFAR10(root='./data', train=True, 
                                        transform=transform_form_0,download=True)
# torch.utils.data.DataLoader()定义的，这个东西来源于torch.utils.data模块
trainloader_0 = torch.utils.data.DataLoader(trainset_0, batch_size=4,
                                          shuffle=True, num_workers=0)
testset_0 = torchvision.datasets.CIFAR10(root='./data', train=False, 
                                       transform=transform_form_0,download=True)
testloader_0 = torch.utils.data.DataLoader(testset_0, batch_size=4,
                                         shuffle=False, num_workers=0)
#%% 定义卷积神经网络
class Net_0(nn.Module):                 # 我们定义网络时一般是继承的torch.nn.Module创建新的子类
    def __init__(self):    
        super(Net_0, self).__init__()   
        self.conv1 = nn.Conv2d(3, 12, 5)       # 添加第一个卷积层,调用了nn里面的Conv2d（通道数，卷积核数，卷积核尺寸5*5）
        self.pool = nn.MaxPool2d(4, 4)        # 最大池化层，（卷积核尺寸，步长）
        self.conv2 = nn.Conv2d(12, 32, 5)      # 同样是卷积层
        self.fc1 = nn.Linear(1152, 260) # 接着三个全连接层
        self.fc2 = nn.Linear(260, 84)
        self.fc3 = nn.Linear(84, 10)
 
    def forward(self, x):           # 这里定义前向传播的方法，为什么没有定义反向传播的方法呢？这其实就涉及到torch.autograd模块了，
        x = F.relu(self.conv1(x))  # F是torch.nn.functional的别名，这里调用了relu函数 F.relu()
        x = self.pool(F.relu(self.conv2(x)))
        x = x.view(-1, 1152 )  # .view( )是一个tensor的方法，使得tensor改变形状。
                                    #  你会发现第一个全连接层的首参数是16*5*5，所以要保证能够相乘，在矩阵乘法之前就要把x调到正确的size
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
...
scheduler = ReduceLROnPlateau(optimizer_0, mode='min', factor=0.1, patience=4, verbose=True)
loss_count_0=[]
ac_count_0 = []
#%% 训练
for epoch in range(20):  # 指定训练一共要循环几个epoch
    running_loss = 0.0  #定义一个变量方便我们对loss进行输出
    for i, data in enumerate(trainloader_0, 0): # enumerate是python的内置函数，既获得索引也获得数据
        inputs, labels = data   # data是从enumerate返回的data，包含数据和标签信息，分别赋值给inputs和labels
        inputs, labels = Variable(inputs), Variable(labels) # 将数据转换成Variable，第二步里面我们已经引入这个模块
        ...
        if i % 2000 == 1999:    
            ...
            correct = 0   # 定义预测正确的图片数，初始化为0
            total = 0     # 总共参与测试的图片数，也初始化为0
            for data in testloader_0:  # 循环每一个batch
                images, labels = data
